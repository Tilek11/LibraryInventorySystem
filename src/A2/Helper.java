...
/**
 * Helper is just some basic functions that can be used with the other classes
 * @author Aeyon Ahmad
 *
 */
...
public class Helper
{
    ...
    public static long timeDifference(String date1, String date2) throws DateFormatException
    {
        ...
        if (!isValidDate(date1) || !isValidDate(date2)) { throw new DateFormatException(); }
    ...
    {
        String s;
        s = "Book (" + book.bookName + ", " + book.valueTag + ") is availble at ";
        
        if (book.rs == null) s += getCurrentDate();
        else s += book.getRentDate();
        
        s += " from library: " + library.libraryName;
